...
#    print "You may need to adjust range/colour axis manually. Also note that colorization by antenna does not show up in pdf output, probably due to a plotms bug in CASA 4.2.2."
    ...
    return pdf, targeta, ampa, phasea, bandpassa, uvr, avgtime

def make_pdf(targeta, ampa, phasea, bandpassa, uvr, avgtime):
...
def plotManual(targeta, ampa, phasea, bandpassa, uvr, avgtime):
...
def main():
    ...
    for vis in vislist:
        ...
        if stuff[0] == 'Y':
            make_pdf(target, amp, phase, bandpass, uvr, avgtime)
        else:
            plotManual(target, amp, phase, bandpass, uvr, avgtime)
