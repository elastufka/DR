...
def staging_info(project_number, user_name):
...
def staging(project_number, user_name):
    sinfo = staging_info(project_number, user_name)
    os.chdir('%s' % sinfo[0])
    tarlist = sinfo[2]
    ...
    for tarball in tarlist:
        if os.path.isfile('/lustre/naasc/sciops/cycle2_release/%s' % tarball):
            #os.system('cp /lustre/naasc/sciops/cycle2_release/%s %s/data/' % (tarball, sinfo[0])) 
            print tarball
        elif os.path.isfile('/lustre/naasc/sciops/cycle1_release/%s' % tarball):
            #os.system('cp /lustre/naasc/sciops/cycle1_release/%s %s/data/' % (tarball, sinfo[0])) 
            print tarball
        elif os.path.isfile('/lustre/naasc/sciops/cycle3_release/%s' % tarball):
            #os.system('cp /lustre/naasc/sciops/cycle3_release/%s %s/data/' % (tarball, sinfo[0]))
            print tarball 
        else:
            sys.exit('ERROR: %s not found in cycleN_release folders!')
        #os.system('tar -xvf %s' % tarball) #EL fix later
